...
describe('connection.ts — getDb() initialization', () => {
  // In CI, Bun runs test files in parallel. Another file may import the real
  // connection singleton and close it. We reset the internal _db to null before
  // our tests so getDb() creates a fresh instance pointing at our temp path.
  let getDb: () => import('bun:sqlite').Database;
  ...
  it('getDb() returns an object with exec and query methods (Database interface)', async () => {
    ...
    getDb = mod.getDb;
    resetDb = (mod as Record<string, unknown>).resetDb as (() => void) | undefined;
    // If the singleton was closed by another test file, reset it
    resetDb?.();
    const db = getDb();
