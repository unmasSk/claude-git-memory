...
## When to Use a Proxy
Use **ProxySQL** (or a managed proxy layer) when: multiple app services share a DB, you need query routing (read/write split), or total connection demand exceeds safe `max_connections`.
